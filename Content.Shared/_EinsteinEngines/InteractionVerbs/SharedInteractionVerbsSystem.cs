...
public abstract partial class SharedInteractionVerbsSystem : EntitySystem
{
    ...
    public bool StartVerb(InteractionVerbPrototype proto, InteractionArgs args, bool force = false)
    {
        ...
        var doAfter = new DoAfterArgs(
            ...
            args.User,
            delay,
            new InteractionVerbDoAfterEvent(proto.ID, args),
            EntityUid.Invalid,
            args.Target,
            args.Used)
        {
            ...
            // <Onyx-InteractionVerbsNullDoAfter-edited>
            BreakOnMove = proto.DoAfter?.BreakOnMove ?? false,
            BreakOnWeightlessMove = proto.DoAfter?.BreakOnWeightlessMove ?? true,
            BreakOnDamage = proto.DoAfter?.BreakOnDamage ?? false,
            DamageThreshold = proto.DoAfter?.DamageThreshold ?? 1,
            MovementThreshold = proto.DoAfter?.MovementThreshold ?? 0.3f,
            DistanceThreshold = proto.DoAfter?.DistanceThreshold ?? 1.5f,
            BlockDuplicate = proto.DoAfter?.BlockDuplicate ?? true,
            CancelDuplicate = proto.DoAfter?.CancelDuplicate ?? true,
            DuplicateCondition = proto.DoAfter?.DuplicateCondition ?? DuplicateConditions.All,
            // </Onyx-InteractionVerbsNullDoAfter-edited>
        };
    ...
    private void CalculateAdvantage(InteractionVerbPrototype proto, ref InteractionArgs args, out bool canPerform)
    {
        ...
        // Corvax Vanilla has no contest system; keep contest-aware prototypes neutral.
