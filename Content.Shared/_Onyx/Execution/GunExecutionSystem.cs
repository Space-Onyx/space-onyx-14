...
public sealed partial class SharedGunExecutionSystem : EntitySystem
{
    ...
    private void TryStartGunExecutionDoafter(EntityUid weapon, EntityUid victim, EntityUid attacker, float gunExecutionTime)
    {
        ...
        if (attacker == victim)
        {
            ShowExecutionPopup("suicide-popup-gun-initial-internal", "suicide-popup-gun-initial-external", attacker, victim, weapon);
        }
        else
        {
            ShowExecutionPopup("execution-popup-gun-initial-internal", "execution-popup-gun-initial-external", attacker, victim, weapon);
        }
    ...
    private void OnDoafterGun(EntityUid uid, GunComponent component, DoAfterEvent args)
    {
        ...
        if (ev.Ammo.Count <= 0)
        {
            ...
            ShowExecutionPopup("execution-popup-gun-empty", "execution-popup-gun-empty", attacker, victim, weapon);
            return;
        }
        ...
        switch (ev.Ammo[0].Shootable)
        {
            ...
            case HitscanAmmoComponent: // Battery hitscan weapons are spawned as hitscan ammo on TakeAmmoEvent
                TryComp<HitscanBasicDamageComponent>(ammoUid, out var hitscanDamage);

                if (hitscanDamage != null)
                {
    ...
    {
        var internalMsg = Loc.GetString(targetMessage, ("attacker", Identity.Entity(attacker, EntityManager)), ("victim", Identity.Entity(victim, EntityManager)), ("weapon", weapon));
        var externalMsg = Loc.GetString(otherMessage, ("attacker", Identity.Entity(attacker, EntityManager)), ("victim", Identity.Entity(victim, EntityManager)), ("weapon", weapon));

        _popup.PopupEntity(internalMsg, attacker, attacker, PopupType.MediumCaution);
        _popup.PopupEntity(externalMsg, attacker, Filter.PvsExcept(attacker), true, PopupType.MediumCaution);
    }
}