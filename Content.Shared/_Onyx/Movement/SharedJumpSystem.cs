...
public abstract partial class SharedJumpSystem : EntitySystem
{
    ...
    public bool TryJump(Entity<JumpComponent> ent)
    {
        ...
        if (!_stamina.TryTakeStamina(ent, staminaCost, source: ent, visual: false))
        {
        ...
        if (table != null &&
            ...
            TryComp(table.Value, out ClimbableComponent? climbable) &&
            !climbing.IsClimbing)
        {
            _climb.StartJumpClimb((ent, climbing), (table.Value, climbable));
        }
    ...
    {
        return ent.Comp.NextJump <= _timing.CurTime &&
