...
using Content.Shared._Onyx.ZLevels.Flight.Components;
...
public abstract partial class CESharedZLevelsSystem
{
    ...
    private void UpdateZPhysics(float frameTime)
    {
        ...
        // Iterate the processing list in reverse so sleep/dormancy removals don't shift
        // indices we're about to visit. The list is kept tight by Wake/Sleep.
        for (var i = _processingBodies.Count - 1; i >= 0; i--)
        {
            var uid = _processingBodies[i];
            ...
            var stableGrounded = restingOnCurrentGround &&
                                 !zPhys.CurrentGroundFromBelowLevel &&
                                 !zPhys.CurrentStickyGround &&
                                 zPhys.CurrentGroundHeight <= GroundContactTolerance &&
                                 MathF.Abs(zPhys.Velocity) <= 0.01f &&
                                 MathF.Abs(zPhys.LocalPosition - zPhys.CurrentGroundHeight) <= 0.01f &&
                                 MathF.Abs(oldHeight - zPhys.LocalPosition) <= 0.01f &&
                                 _timing.CurTime >= zPhys.StartupSuppressedUntil;
            UpdateDormancy(uid, stableGrounded);
        }
    ...
    {
        if (_mapQuery.TryComp(mapUid, out var mapComponent))
        {
    ...
    protected bool TryResolveGridAtWorldPositionOnMap(EntityUid mapUid, Vector2 worldPos, out EntityUid gridUid, out MapGridComponent gridComp)
    {
        ...
        foreach (var gridEntity in _map.GetAllGrids(mapComponent.MapId))
        {
            var uid = gridEntity.Owner;
