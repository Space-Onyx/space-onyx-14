...
namespace Content.Shared._Onyx.ZLevels.Core;

public readonly record struct CEZLevelOpening(EntityUid GridUid, Vector2i GridIndices, Vector2 Center, float Distance);
...
public sealed class CMUZLevelOpeningCache
{
        ...
        ITileDefinitionManager tileDefinition)
    {
        ...
        {
            GetTileSearchBounds(grid, worldAabb, transform, out var startX, out var endX, out var startY, out var endY);
        ...
        bool edgeOnly = true)
    {
        ...
        foreach (var grid in gridScratch)
        {
            GetTileSearchBounds(grid, searchBounds, transform, out var startX, out var endX, out var startY, out var endY);
                ...
                for (var chunkY = startChunk.Y; chunkY <= endChunk.Y; chunkY++)
                {
                    ...
                    ForEachOpeningTileInBounds(
                        ...
                        openingTile =>
                        {
                            ...
                            openings.Add(new CEZLevelOpening(grid.Owner, openingTile, center, MathF.Sqrt(distanceSquared)));
                            return false;
                        });
                }
            }
    ...
    private static void GetTileSearchBounds(
        ...
        out int endY)
    {
        var inverse = transform.GetInvWorldMatrix(grid.Owner);
        var localBottomLeft = Vector2.Transform(worldBounds.BottomLeft, inverse);
        var localBottomRight = Vector2.Transform(worldBounds.BottomRight, inverse);
        var localTopLeft = Vector2.Transform(worldBounds.TopLeft, inverse);
        var localTopRight = Vector2.Transform(worldBounds.TopRight, inverse);

        var minX = MathF.Min(MathF.Min(localBottomLeft.X, localBottomRight.X), MathF.Min(localTopLeft.X, localTopRight.X));
        var maxX = MathF.Max(MathF.Max(localBottomLeft.X, localBottomRight.X), MathF.Max(localTopLeft.X, localTopRight.X));
        var minY = MathF.Min(MathF.Min(localBottomLeft.Y, localBottomRight.Y), MathF.Min(localTopLeft.Y, localTopRight.Y));
        var maxY = MathF.Max(MathF.Max(localBottomLeft.Y, localBottomRight.Y), MathF.Max(localTopLeft.Y, localTopRight.Y));

        startX = (int) MathF.Floor(minX) - 1;
        endX = (int) MathF.Ceiling(maxX) + 1;
        startY = (int) MathF.Floor(minY) - 1;
        endY = (int) MathF.Ceiling(maxY) + 1;
    }
