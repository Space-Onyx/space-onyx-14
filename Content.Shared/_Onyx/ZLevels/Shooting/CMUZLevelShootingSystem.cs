...
using Content.Shared._Onyx.Input;
...
namespace Content.Shared._Onyx.ZLevels.Shooting;
...
public sealed partial class CMUZLevelShootingSystem : EntitySystem
{
    ...
    private float _zLevelVisualOffset = 0.3f;
    ...
    public override void Initialize()
    {
        ...
        Subs.CVar(_config, SharedCCVars.CEZLevelsRenderOffset, v => _zLevelVisualOffset = MathF.Max(0f, v), true);
        ...
        bool requireReadyGunForLookUp = true)
    {
        ...
        var shooterMap = shooterXform.MapUid;
        ...
        if (shooterMap == null ||
            !_zLevels.TryResolveLinkedTarget(
                shooterXform.GridUid,
                shooterMap.Value,
                offset,
                fromMap.Position,
                out var targetMapUid,
                out var targetFromWorld) ||
            !_zLevels.TryResolveLinkedTarget(
                shooterXform.GridUid,
                shooterMap.Value,
                offset,
                clampedTo,
                out var targetToMapUid,
                out var targetToWorld) ||
            targetMapUid != targetToMapUid ||
            !TryComp<MapComponent>(targetMapUid, out var targetMapComp))
        {
        ...
        var openingFrom = offset < 0 ? fromMap.Position : targetFromWorld;
        var openingTo = offset < 0 ? clampedTo : targetToWorld;
        ...
        var projectileFrom = targetFromWorld + renderShift;
        var projectileTo = targetToWorld + renderShift;
    ...
    private Vector2 GetShooterRenderShift(EntityUid shooter, int offset)
    {
        ...
        return gridRotation.ToWorldVec() * _zLevelVisualOffset * offset;
    }
