...
public sealed partial class VampireSystem
{
    ...
    [Dependency] private SharedBodySystem _bodyGraph = default!;
    ...
    private void OnRejuvenate(Entity<VampireComponent> ent, ref VampireRejuvenateActionEvent args)
    {
        ...
        _stamina.TakeStaminaDamage(args.Performer, -_stamina.GetStaminaDamage(args.Performer));
    ...
    private void RemoveInternalBleeding(EntityUid uid)
    {
        foreach (var part in _bodyGraph.GetBodyChildren(uid))
        {
            foreach (var wound in _wounds.GetWounds(part.Id).ToArray())
            {
                if (HasComp<WoundInternalBleedingComponent>(wound))
                    _wounds.RemoveWound(wound.AsNullable());
            }
        }
    }
