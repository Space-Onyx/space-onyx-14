...
public sealed partial class AbductorSystem : SharedAbductorSystem
{
    ...
    [Dependency] private ItemSwitchSystem _itemSwitch = default!;
    ...
    {
        if (component.Armor is { } armor
            && TryComp<ItemSwitchComponent>(GetEntity(armor), out var itemSwitch))
            _itemSwitch.Switch((GetEntity(armor), itemSwitch), args.Mode.ToString(), args.Actor);
    }
            ...
            && TryComp<AbductorExperimentatorComponent>(experimentatorId, out var experimentatorComp))
        {
            ...
            if (victim != default && TryComp(victim, out AbductorVictimComponent? victimComp))
            {
                if (victimComp.Implanted
                    ...
                    && !condition.AbductedEntities.Contains(GetNetEntity(victim)))
                {
                    condition.AbductedEntities.Add(GetNetEntity(victim));
                    condition.Abducted++;
                }
                ...
                {
                    _container.Remove(victim, container, force: true);
    ...
    private void OnAttractBuiMsg(Entity<AbductorConsoleComponent> ent, ref AbductorAttractBuiMsg args)
    {
        ...
        _audioSystem.PlayPvs(new SoundPathSpecifier("/Audio/_Onyx/Abductor/alien_teleport.ogg"), effectEnt);
        ...
        _audioSystem.PlayPvs(new SoundPathSpecifier("/Audio/_Onyx/Abductor/alien_teleport.ogg"), effect);
    ...
    protected override void UpdateGui(NetEntity? target, Entity<AbductorConsoleComponent> computer)
    {
        ...
        var armorLock = false;
        var armorMode = AbductorArmorMode.Stealth;

        if (computer.Comp.Armor != null)
        {
            if (HasComp<UnremoveableComponent>(GetEntity(computer.Comp.Armor.Value)))
                armorLock = true;
            if (TryComp<ItemSwitchComponent>(GetEntity(computer.Comp.Armor.Value), out var switchVest) && Enum.TryParse<AbductorArmorMode>(switchVest.State, ignoreCase: true, out var State))
                armorMode = State;
        }
