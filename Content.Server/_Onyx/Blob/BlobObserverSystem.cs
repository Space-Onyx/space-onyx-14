...
public sealed partial class BlobObserverSystem : SharedBlobObserverSystem
{
    ...
    private void OnChemSelected(EntityUid uid, BlobObserverComponent component, BlobChemSwapPrototypeSelectedMessage args)
    {
        ...
        if (blobCoreComponent.CurrentChem == args.SelectedId ||
            !blobCoreComponent.ChemСolors.ContainsKey(args.SelectedId) ||
            !blobCoreComponent.ChemDamageDict.ContainsKey(args.SelectedId))
            return;
        ...
        if (!ChangeChem(uid, args.SelectedId, component))
            _blobCoreSystem.ChangeBlobPoint(component.Core.Value, blobCoreComponent.SwapChemCost);
    }
    ...
    private bool ChangeChem(EntityUid uid, BlobChemType newChem, BlobObserverComponent component)
    {
        ...
        if (!_blobCoreSystem.ChangeChem(core, newChem, blobCoreComponent))
            return false;
        ...
        BlobSplitCoreActionEvent args)
    {
        ...
        foreach (var tileref in centerTile)
        {
            ...
            {
                if (!_tileQuery.TryComp(ent, out var tile) || tile.Core?.Owner != uid)
                    continue;
                blobTile = ent;
                break;
            }
        ...
        {
            QueueDel(newCore);
            _blobCoreSystem.ChangeBlobPoint((uid, blobCoreComponent), blobCoreComponent.SplitCoreCost);
            args.Handled = true;
            return;
        }
        ...
        blobCoreComponent.Actions.Remove(args.Action.Owner);
        ...
        BlobCreateBlobbernautActionEvent args)
    {
        ...
        if (blobTile == null || blobTile.Value.Comp.Core?.Owner != uid ||
            !TryComp<BlobFactoryComponent>(blobTile, out var blobFactoryComponent))
        {
