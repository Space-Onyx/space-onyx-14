...
public sealed partial class BlobCoreActionSystem : SharedBlobCoreActionSystem
{
    ...
    private void BlobInteract(Entity<BlobObserverComponent> observer, Entity<BlobCoreComponent> core, InteractEvent args, bool spendPoints)
    {
        ...
        var node = _blobCoreSystem.GetNearNode(location, core, core.Comp.TilesRadiusLimit);
        ...
        {
            if (spendPoints)
                _blobCoreSystem.ChangeBlobPoint(core, cost);
            return;
        }

        if (targetTileEmpty)
            _mapSystem.SetTile(gridUid.Value, grid, location, new Tile(_tileDefinitionManager["Plating"].TileId));

        core.Comp.NextAction = _gameTiming.CurTime + GCd + TimeSpan.FromSeconds(Math.Abs(core.Comp.GrowRate));
    }

    private EntityUid? FindNearBlobTile(EntityCoordinates coords, Entity<MapGridComponent> grid, Entity<BlobCoreComponent> core)
    {
        ...
        foreach (var indices in adjacentTiles)
        {
            ...
            if (uid == null || _tileQuery.Comp(uid.Value).Core?.Owner != core.Owner)
                continue;
