...
namespace Content.Server._Onyx.Blob.GameTicking;
...
public sealed partial class BlobRuleSystem : GameRuleSystem<BlobRuleComponent>
{
    ...
    private static readonly ProtoId<AlertLevelPrototype> StationAlertCritical = "DeltaNuke";
        ...
        long blobTilesCount)
    {
        ...
        var stage = blobRuleComp.StationStages.GetValueOrDefault(stationUid, BlobStage.Default);
        switch (stage)
        {
            case BlobStage.Default when blobTilesCount >= (stationUid.Comp?.StageBegin ?? StationBlobConfigComponent.DefaultStageBegin):
                stage = BlobStage.Begin;
                blobRuleComp.StationStages[stationUid] = stage;
