...
public sealed partial class VampireSystem
{
    ...
    private static readonly SoundSpecifier BestiaScream = new SoundPathSpecifier("/Audio/_Onyx/Effects/Vampire/creepyshriek.ogg");
    ...
    private void OnDestruction(EntityUid uid, VampireCoffinComponent coffin, DestructionAttemptEvent args)
    {
        ...
        var nearbyEntities = _entityLookup.GetEntitiesInRange<MobStateComponent>(coordinates, 5f)
            .Where(entity => !_mobState.IsDead(entity.Owner) && !IsSynthetic(entity.Owner))
            .ToList();
    ...
    private void OnMarkPrey(Entity<VampireComponent> ent, ref VampireMarkPreyActionEvent args)
    {
        ...
        _movementMod.TryUpdateMovementSpeedModDuration(target, VampireSlowdown, TimeSpan.FromSeconds(markDuration), 0.5f);
