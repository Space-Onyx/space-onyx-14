...
public sealed partial class VampireSystem : SharedVampireSystem
{
    ...
    private static readonly ProtoId<TagPrototype> VampireBloodTag = "VampireBloodSource";
    ...
    {
        "Blood", "CopperBlood", "InsectBlood", "SulfurBlood", "AmmoniaBlood"
    };
        ...
        if (TryComp<BodyComponent>(vampire, out var body) && body.Organs != null)
        {
            foreach (var organ in body.Organs.ContainedEntities)
            {
                if (TryComp<MetabolizerComponent>(organ, out var meta))
                {
                    ...
                    _metabolism.SetMetabolizerTypes((organ, meta), VampireComponent.MetabolizerVampire);
                }
            }
        }
    ...
    private void RestoreOriginalState(Entity<VampireComponent> vampire, VampireOriginalStateComponent state)
    {
        ...
        foreach (var (organ, types) in state.OriginalMetabolizerTypes)
        {
            if (TryComp<MetabolizerComponent>(organ, out var meta))
            {
                _metabolism.SetMetabolizerTypes((organ, meta), types.ToArray());
            }
        }
    ...
    private bool TryDrink(Entity<VampireComponent> ent, EntityUid target)
    {
        ...
        if (!_interaction.InRangeUnobstructed(ent.Owner, target, popup: true) || IsSynthetic(target))
            return false;
        ...
        if (!_tag.HasTag(target, VampireBloodTag))
        {
            _popup.PopupEntity(Loc.GetString("vampire-blooddrink-not-sentient"), ent, ent, PopupType.SmallCaution);
            return false;
        }

        if (!HasDrinkableBlood(ent.Owner, target))
        {
            _popup.PopupEntity(Loc.GetString("vampire-blooddrink-synthetic"), ent, ent, PopupType.SmallCaution);
            return false;
        }
    ...
    private bool HasDrinkableBlood(EntityUid vampire, EntityUid target)
    {
        ...
        {
            if (BloodProto.Any(b => b.Id == reagent.Reagent.Prototype))
                return true;
        }
    ...
    private void DrinkDoAfter(EntityUid uid, VampireComponent component, ref VampireDrinkingBloodDoAfterEvent args)
    {
        ...
        if (!HasDrinkableBlood(uid, target))
        {
            _popup.PopupEntity(Loc.GetString("vampire-blooddrink-synthetic"), uid, uid, PopupType.SmallCaution);
            return;
        }
        ...
        if (_tag.HasTag(target, VampireBloodTag) && !HasComp<DnaModifiedComponent>(target))
        ...
        if (body.Organs != null)
        {
            foreach (var organ in body.Organs.ContainedEntities)
            {
                if (HasComp<StomachComponent>(organ)
                    && _stomach.CanTransferSolution(organ, ingestedSolution))
                    return _stomach.TryTransferSolution(organ, ingestedSolution);
            }
        }

        foreach (var (organ, _) in _bodyGraph.GetBodyOrgans(uid))
        {
            if (HasComp<StomachComponent>(organ)
                && _stomach.CanTransferSolution(organ, ingestedSolution))
                return _stomach.TryTransferSolution(organ, ingestedSolution);
    ...
    private bool IsSynthetic(EntityUid uid)
    {
        if (!TryComp<BodyComponent>(uid, out var body))
            return false;

        return _body.TryGetOrgansWithComponent<MechanicalOrganComponent>((uid, body), out _);
    }
