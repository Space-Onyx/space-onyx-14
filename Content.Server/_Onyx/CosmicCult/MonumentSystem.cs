...
using Content.Server._Onyx.CosmicCult.EntitySystems;
...
using Content.Shared._Onyx.Targeting; // Shitmed Change
...
public sealed partial class MonumentSystem : SharedMonumentSystem
{
        ...
        while (finaleQuery.MoveNext(out var uid, out var comp, out var monuComp))
        {
            if (_timing.CurTime >= monuComp.CheckTimer)
            {
                ...
                foreach (var entity in entities)
                    ...
                    {
                        _damage.TryChangeDamage(entity, monuComp.MonumentHealing * cultComp.ShitMedHeal);
                    }
    ...
    public void UpdateMonumentProgress(Entity<MonumentComponent> ent, Entity<CosmicCultRuleComponent> cult) =>
        ent.Comp.CurrentProgress = ent.Comp.TotalEntropy + cult.Comp.TotalCult * _config.GetCVar(CCVars.CosmicCultistEntropyValue);
    ...
    public void UpdateMonumentReqsForTier(Entity<MonumentComponent> monument, int tier)
    {
        ...
        var numberOfCrewForTier3 = Math.Round((double) cult.Comp.TotalCrew / 100 * _config.GetCVar(CCVars.CosmicCultTargetConversionPercent)); // 40% of current pop
        var difficultyMultiplier = _config.GetCVar(CCVars.CosmicCultistDifficultyMultiplier);
        switch (tier)
        {
            case 1:
                monument.Comp.ProgressOffset = 0;
                monument.Comp.TargetProgress = (int) (difficultyMultiplier * numberOfCrewForTier3 / 2 * _config.GetCVar(CCVars.CosmicCultistEntropyValue));
                break;
            case 2:
                monument.Comp.ProgressOffset = (int) (difficultyMultiplier * numberOfCrewForTier3 / 2 * _config.GetCVar(CCVars.CosmicCultistEntropyValue)); //reset the progress offset
                monument.Comp.TargetProgress = (int) (difficultyMultiplier * numberOfCrewForTier3 * _config.GetCVar(CCVars.CosmicCultistEntropyValue));
                break;
            case 3:
                monument.Comp.ProgressOffset = (int) (difficultyMultiplier * numberOfCrewForTier3 * _config.GetCVar(CCVars.CosmicCultistEntropyValue));
                monument.Comp.TargetProgress = (int) (difficultyMultiplier * numberOfCrewForTier3 * _config.GetCVar(CCVars.CosmicCultistEntropyValue)); //removed offset; replaced with timer
                break;
        }
    }
