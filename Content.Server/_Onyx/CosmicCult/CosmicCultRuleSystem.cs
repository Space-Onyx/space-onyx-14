...
using Content.Shared._Onyx.CosmicCult.Prototypes;
...
public sealed partial class CosmicCultRuleSystem : GameRuleSystem<CosmicCultRuleComponent>
{
    ...
    private readonly SoundSpecifier _briefingSound = new SoundPathSpecifier("/Audio/_Onyx/CosmicCult/antag_cosmic_briefing.ogg");
    private readonly SoundSpecifier _deconvertSound = new SoundPathSpecifier("/Audio/_Onyx/CosmicCult/antag_cosmic_deconvert.ogg");
    private readonly SoundSpecifier _tier3Sound = new SoundPathSpecifier("/Audio/_Onyx/CosmicCult/tier3.ogg");
    private readonly SoundSpecifier _tier2Sound = new SoundPathSpecifier("/Audio/_Onyx/CosmicCult/tier2.ogg");
    private readonly SoundSpecifier _monumentAlert = new SoundPathSpecifier("/Audio/_Onyx/CosmicCult/tier_up.ogg");
    ...
    public override void Initialize()
    {
        ...
        Subs.CVar(_config,
            CCVars.CosmicCultT2RevealDelaySeconds,
            value => _t2RevealDelay = TimeSpan.FromSeconds(value),
            true);
        Subs.CVar(_config,
            CCVars.CosmicCultT3RevealDelaySeconds,
            value => _t3RevealDelay = TimeSpan.FromSeconds(value),
            true);
        Subs.CVar(_config,
            CCVars.CosmicCultFinaleDelaySeconds,
            value => _finaleDelay = TimeSpan.FromSeconds(value),
            true);
        Subs.CVar(_config,
            CCVars.CosmicCultStewardVoteTimer,
    ...
    public void TryStartCult(EntityUid uid, Entity<CosmicCultRuleComponent> rule)
    {
        ...
        cultComp.HolyWeaknessCaptured = true;
        cultComp.HadHolyWeakness = false;
        ...
        cultComp.StoredDamageContainer = Comp<InjurableComponent>(uid).DamageContainer ?? "Biological";
    ...
    public void CosmicConversion(EntityUid converter, EntityUid uid)
    {
        ...
        CaptureAndApplyHolyWeakness(uid, cultComp);
    ...
    private void OnComponentShutdown(Entity<CosmicCultComponent> uid, ref ComponentShutdown args)
    {
        ...
        RestoreHolyWeakness(uid);

        var injurable = EnsureComp<InjurableComponent>(uid);
        injurable.DamageContainer = uid.Comp.StoredDamageContainer;
        Dirty(uid, injurable);
        ...
        UpdateCultData(cosmicGamerule.MonumentInGame);
        _movementSpeed.RefreshMovementSpeedModifiers(uid.Owner);
    }

    public void EnsureHolyWeakness(EntityUid uid, CosmicCultComponent cultComp)
    {
        CaptureAndApplyHolyWeakness(uid, cultComp);
    }

    private void CaptureAndApplyHolyWeakness(EntityUid uid, CosmicCultComponent cultComp)
    {
        if (!cultComp.HolyWeaknessCaptured)
        {
            cultComp.HadHolyWeakness = TryComp<WeakToHolyComponent>(uid, out var existing);
            cultComp.PreviousAlwaysTakeHoly = existing?.AlwaysTakeHoly ?? false;
            cultComp.HolyWeaknessCaptured = true;
        }

        var weakness = EnsureComp<WeakToHolyComponent>(uid);
        weakness.AlwaysTakeHoly = true;
        Dirty(uid, weakness);
        Dirty(uid, cultComp);
    }

    private void RestoreHolyWeakness(Entity<CosmicCultComponent> ent)
    {
        if (!ent.Comp.HolyWeaknessCaptured || !ent.Comp.HadHolyWeakness)
        {
            RemComp<WeakToHolyComponent>(ent);
            return;
        }

        var weakness = EnsureComp<WeakToHolyComponent>(ent);
        weakness.AlwaysTakeHoly = ent.Comp.PreviousAlwaysTakeHoly;
        Dirty(ent.Owner, weakness);
    }
    #endregion
}
