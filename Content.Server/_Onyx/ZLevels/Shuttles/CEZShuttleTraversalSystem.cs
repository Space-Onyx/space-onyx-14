...
using Content.Shared._Onyx.ZLevels.Shuttles.Components;
...
namespace Content.Server._Onyx.ZLevels.Shuttles;
...
/// </summary>
public sealed partial class CEZShuttleTraversalSystem : EntitySystem
{
    [Dependency] private CEZLevelsSystem _zLevels = default!;
    [Dependency] private CEZShuttleRoofSystem _roof = default!;
    ...
    public EntityUid ResolveRoot(EntityUid grid)
    {
        ...
        return linked.PeerGrids.TryGetValue(0, out var root) ? root : grid;
    }

    public void WriteConsoleState(EntityUid root, ShuttleMapInterfaceState state)
    {
        root = ResolveRoot(root);
        if (TryComp<CEZShuttleTraversalComponent>(root, out var traversal))
        {
            state.ZTraversalState = traversal.State;
            state.ZTraversalTime = traversal.StateTime;
        }
        else
        {
        ...
        GetFlyOptions(root, out state.CanFlyUp, out state.CanFlyDown);
    }
