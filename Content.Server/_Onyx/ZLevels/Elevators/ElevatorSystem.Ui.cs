...
namespace Content.Server._Onyx.ZLevels.Elevators;
...
public sealed partial class ElevatorSystem
{
    ...
    {
        if (!TryGetController(elevatorId, out var controller) || !controller.Value.Comp.CabInitialized)
        {
    ...
    private void PushPanelState(Entity<ElevatorPanelComponent> ent)
    {
        ...
        if (TryGetController(ent.Comp.ElevatorId, out var controller) && controller.Value.Comp.CabInitialized)
        {
            var comp = controller.Value.Comp;
