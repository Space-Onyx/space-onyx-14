...
using Content.Shared._Onyx.ZLevels.Surveillance;
...
public sealed partial class CEZCameraViewSubscriptionSystem : EntitySystem
{
    ...
    private void AddViewSource(EntityUid uid, ICommonSession subscriber)
    {
        ...
        RefreshSessionEyes((uid, subscriptions), subscriber, GetZViewTargets((uid, Transform(uid))));
    }
    ...
    private bool EyesMatchTargets(List<EntityUid> eyes, List<ZViewTarget> targets)
    {
        ...
        {
            if (TerminatingOrDeleted(eyes[i]))
                return false;

            if (Transform(eyes[i]).MapUid != targets[i].MapUid)
            {
                return false;
            }
        }
    ...
    private bool TryResolveCameraZViewTarget(Entity<TransformComponent> camera, int depthOffset, out ZViewTarget target)
    {
            ...
            TryComp<CEZLinkedGridComponent>(gridUid, out var linked))
        {
            ...
            if (Transform(peerGridUid).MapUid is { } peerMapUid)
            {
