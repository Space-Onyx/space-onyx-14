...
public sealed partial class CEZLevelsSystem
{
    ...
    private readonly Dictionary<ItemTileKey, ItemTileSample> _itemTileSamples = new();
    private readonly Queue<EntityUid> _pendingItemTransitions = new();
    private readonly HashSet<EntityUid> _pendingItemTransitionSet = new();

    [ViewVariables]
    public int PendingItemTransitionCount => _pendingItemTransitionSet.Count;

    private readonly record struct ItemTileKey(EntityUid Grid, Vector2i Tile);

    private struct ItemTileSample
    {
        public float GroundHeight;
    }

    private enum PendingTransitionResult : byte
    {
        Completed,
        Removed,
        BudgetExhausted,
    ...
    private void InitItems()
    {
        ...
        SubscribeLocalEvent<ItemComponent, ThrowEvent>(OnItemThrown);
    ...
    private void OnItemThrown(Entity<ItemComponent> ent, ref ThrowEvent args)
    {
    ...
    {
        _pendingItemTransitionSet.Remove(item);
    ...
    {
        _itemTileSamples.Clear();
        ProcessPendingItemTransitions();
        ...
        {
            if (zItem.PendingTransition != 0)
            {
                QueuePendingItemTransition(uid);
                continue;
            }
            ...
            if (!TryGetItemTileContext(uid, xform, out var tileKey, out var grid))
            {
                FinishItemZFall(uid, zItem);
                continue;
            }

            if (!_itemTileSamples.TryGetValue(tileKey, out var tileSample))
            {
                tileSample.GroundHeight = ComputeItemGroundHeight(tileKey, grid);
                _itemTileSamples[tileKey] = tileSample;
            }

            TryComp<PhysicsComponent>(uid, out var physics);
            var onGround = physics is null || physics.BodyStatus == BodyStatus.OnGround;

            var groundHeight = tileSample.GroundHeight;
            ...
            {
                if (CanItemDescendFromCurrentTile(uid, xform))
                {
                    if (!CanProcessZLevelTransition(uid, -1))
                    {
                        zItem.LocalPosition = 0f;
                        zItem.PendingTransition = -1;
                        QueuePendingItemTransition(uid);
                        DirtyItemZVisuals(uid, zItem, oldLocalPosition);
                        continue;
                    }

                    if (TryMoveDown(uid, bypassPassability: true))
                    {
            ...
            {
                if (zItem.ZVelocity > 0f && !HasTileAbove(uid))
                {
                    if (!CanProcessZLevelTransition(uid, 1))
                    {
                        zItem.LocalPosition = 1f;
                    ...
                    if (TryMoveUp(uid, bypassPassability: true))
                        zItem.LocalPosition -= 1f;
                    else
                    {
                        zItem.LocalPosition = 1f;
    ...
    /// </summary>
    private bool TryGetItemTileContext(
        EntityUid uid,
        ...
        out MapGridComponent grid)
    {
        ...
        key = new ItemTileKey(gridUid, _map.WorldToTile(gridUid, grid, worldPos));
        return true;
    }

    private float ComputeItemGroundHeight(ItemTileKey key, MapGridComponent grid)
    {

        var found = false;
        var peakHeight = 0f;
        var enumerator = _map.GetAnchoredEntitiesEnumerator(key.Grid, grid, key.Tile);
        ...
        if (_map.TryGetTileRef(key.Grid, grid, key.Tile, out var tileRef) && !tileRef.Tile.IsEmpty)
            return 0f;

        return -1f;
    }

    private void ProcessPendingItemTransitions()
    {
        var count = _pendingItemTransitions.Count;
        for (var i = 0; i < count; i++)
        {
            var uid = _pendingItemTransitions.Dequeue();
            _pendingItemTransitionSet.Remove(uid);

            if (!TryComp<CEZItemPhysicsComponent>(uid, out var zItem) || zItem.PendingTransition == 0 ||
                !Exists(uid))
            {
                continue;
            }
            ...
            var oldLocalPosition = zItem.LocalPosition;
            var result = TryCommitPendingItemTransition(uid, zItem, xform);
            if (result == PendingTransitionResult.BudgetExhausted)
            {
                QueuePendingItemTransition(uid);
                break;
            }
            if (HasComp<CEZItemPhysicsComponent>(uid))
                DirtyItemZVisuals(uid, zItem, oldLocalPosition);
        }
    }

    private void QueuePendingItemTransition(EntityUid uid)
    {
        if (_pendingItemTransitionSet.Add(uid))
            _pendingItemTransitions.Enqueue(uid);
    }

    private PendingTransitionResult TryCommitPendingItemTransition(
        EntityUid uid,
        CEZItemPhysicsComponent zItem,
        TransformComponent xform)
    {
        var direction = zItem.PendingTransition;
        if (direction < 0)
        {
            if (!CanItemDescendFromCurrentTile(uid, xform))
            {
                zItem.PendingTransition = 0;
                FinishItemZFall(uid, zItem);
                return PendingTransitionResult.Removed;
            }
        }
        else
        {
            if (HasTileAbove(uid) || zItem.ZVelocity <= 0f)
            {
                zItem.PendingTransition = 0;
                zItem.LocalPosition = 1f;
                zItem.ZVelocity = 0f;
                return PendingTransitionResult.Completed;
            }
        }

        if (!CanProcessZLevelTransition(uid, direction))
            return PendingTransitionResult.BudgetExhausted;

        var moved = direction < 0
            ? TryMoveDown(uid, bypassPassability: true)
            : TryMoveUp(uid, bypassPassability: true);
        zItem.PendingTransition = 0;
        if (!moved)
        {
            if (direction < 0)
                ...
                zItem.ZVelocity = 0f;
            return direction < 0 ? PendingTransitionResult.Removed : PendingTransitionResult.Completed;
        }
        ...
        if (direction < 0)
        {
            zItem.LocalPosition += 1f;
            zItem.HadZFall = true;
            var fallEvent = new CEZLevelFallMapEvent();
            RaiseLocalEvent(uid, ref fallEvent);
        }
        else
        {
            zItem.LocalPosition -= 1f;
