...
public sealed partial class CEZLevelsSystem
{
    ...
    private readonly List<CEZLevelOpening> _probeOpeningCandidates = new();
    ...
    private void BuildWantedProbeDepths(Entity<TransformComponent> ent, Vector2 globalPos, List<int> depths, bool forceUpperPreview)
    {
        ...
        for (var i = 1; i <= CESharedZLevelsSystem.MaxZLevelsBelowRendering && remainingProbes > 0; i++)
        {
            ...
            if (!HasZOpeningPath(ent, globalPos, -i, requireVisibleFirstStep: true))
                break;
        ...
        bool requireVisibleFirstStep = false)
    {
        ...
        for (var depth = 0; depth != targetDepth; depth += step)
        {
            ...
            var hasOpening = requireVisibleFirstStep && depth == 0
                ? HasVisibleZOpeningNear(checkingMap, checkingPos)
                : HasZOpeningNear(checkingMap, checkingPos);
            if (!hasOpening)
                return false;
        }
    ...
    private bool HasVisibleZOpeningNear(EntityUid map, Vector2 globalPos)
    {
        ...
        if (mapXform.MapID == MapId.Nullspace)
            return true;

        if (!_map.TryFindGridAt(mapXform.MapID, globalPos, out var gridUid, out var grid))
            return true;

        var center = _map.WorldToTile(gridUid, grid, globalPos);
        if (HasOpeningInTileBounds(new Entity<MapGridComponent>(gridUid, grid), center, center))
            return true;

        _probeOpeningCandidates.Clear();
        FindOpeningCentersNear(
            mapXform.MapID,
            globalPos,
            ZProbeOpeningTileRadius * grid.TileSize * MathF.Sqrt(2f),
            _probeOpeningCandidates,
            _probeOpeningGrids);

        if (_probeOpeningCandidates.Count == 0)
            return false;
        if (_probeOpeningCandidates.Count > MaxProbeOpeningLosChecks)
            return true;

        var origin = new MapCoordinates(globalPos, mapXform.MapID);
        foreach (var opening in _probeOpeningCandidates)
        {
            var target = new MapCoordinates(opening.Center, mapXform.MapID);
    ...
    private bool HasZOpeningNear(EntityUid map, Vector2 globalPos)
    {
        ...
        return HasOpeningInTileBounds(new Entity<MapGridComponent>(gridUid, grid), start, end);
    }
    ...
    private bool TryResolveViewerMap(Entity<TransformComponent> ent, int depthOffset, out CEZLevelViewerTarget target)
    {
            ...
            TryComp<CEZLinkedGridComponent>(gridUid, out var linked))
        {
            ...
            if (linked.PeerGrids.TryGetValue(targetDepth, out var peerGridUid) &&
                Transform(peerGridUid).MapUid is { } peerMapUid)
            {
