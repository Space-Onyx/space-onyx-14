...
using Content.Server._Onyx.ZLevels.Power;
...
/// When grids are linked, the lowest-depth grid drives position, rotation,
...
public sealed partial class CEZLevelsSystem
{
    ...
    /// </summary>
    private void ApplyLinkage(EntityUid mapNetwork, EntityUid linkNetwork, Dictionary<int, EntityUid> gridsByDepth)
    {
        ...
        foreach (var (_, gridUid) in gridsByDepth)
        {
            if (!TryComp<CEZLinkedGridComponent>(gridUid, out var existing) ||
                existing.LinkNetwork == linkNetwork)
            {
                continue;
            }
            ...
            if (IsLowestDepth(existing))
                _lastSyncedVelocity.Remove(existing.LinkNetwork);
        }

        foreach (var (depth, gridUid) in gridsByDepth)
        {
            var comp = EnsureComp<CEZLinkedGridComponent>(gridUid);
            comp.ZNetwork = mapNetwork;
            comp.LinkNetwork = linkNetwork;
            comp.Depth = depth;
    ...
    /// <summary>
    /// Snaps every peer grid onto the lowest-depth leader's local transform at linkage time.
    ...
    {
        var leaderDepth = int.MaxValue;
        var leaderUid = EntityUid.Invalid;
        foreach (var (depth, grid) in gridsByDepth)
        {
        ...
        foreach (var (depth, peerUid) in gridsByDepth)
        {
            ...
            var peerXform = Transform(peerUid);
    ...
    private void OnLinkedGridRemoved(Entity<CEZLinkedGridComponent> ent, ref ComponentRemove args)
    {
        ...
        // matching peers against a velocity from a long-dead network. Only the lowest-depth grid
        // owns the entry, so cleanup is keyed off leader removal.
        if (IsLowestDepth(ent.Comp))
            _lastSyncedVelocity.Remove(ent.Comp.LinkNetwork);
    }
    ...
    private void UpdateGridSync(float frameTime)
    {
        ...
        try
        {
            ...
            {
                // The lowest connected depth is authoritative; connector networks need not contain depth 0.
                if (!IsLowestDepth(linked))
                    continue;
                ...
                _lastSyncedVelocity.TryGetValue(linked.LinkNetwork, out var lastSynced);
                ...
                _lastSyncedVelocity[linked.LinkNetwork] = (body.LinearVelocity, body.AngularVelocity);
            }
        }
        finally
        {
    ...
    private static bool IsLowestDepth(CEZLinkedGridComponent linked)
    {
        ...
        out PhysicsComponent? peerBody)
    {
        ...
        if (!TryComp<CEZLinkedGridComponent>(peerUid, out var peerLinked) ||
            peerLinked.LinkNetwork != leaderLinked.LinkNetwork)
            return false;
