...
public sealed partial class CEMultizAtmosPipeAdapterNode : PipeNode
{
        ...
        IEntityManager entMan)
    {
        ...
        var gridTile = mapSystem.TileIndicesFor(gridEnt, xform.Comp.Coordinates);

        // Convert our tile to world coords so we can reproject onto peers that may have
        // different grid transforms within their maps.
        var sourceTileWorld = mapSystem.GridTileToWorldPos(gridUid, gridEnt.Comp, gridTile);
        ...
        foreach (var depthOffset in DepthOffsets)
        {
            ...
            foreach (var node in NodeHelpers.GetNodesInTile(nodeQuery, (peerGridUid, peerGrid), peerTile, mapSystem))
            {
