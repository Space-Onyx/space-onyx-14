...
public sealed partial class MedipenRefillerSystem : EntitySystem
{
    ...
    [Dependency] private EntityManager _entityManage = default!;
    [Dependency] private MedipenSystem _medipen = default!;

    public const string InputSlotName = "beakerSlot";
    public const string MedipenSlotName = "medipenSlot";

    public const string MedipenSolutionName = "hypospray";
    ...
    public void OnMedipenFill(Entity<MedipenRefillerComponent> entity, ref MedipenRefillerFillMedipenMessage args)
    {
        ...
        if (!_solutionContainer.TryGetSolution(medipenSlot.Value, MedipenSolutionName, out var medipenSolutionComp, out var medipenSolution) ||
            !TryComp<MedipenComponent>(medipenSlot.Value, out var medipenComp))
            return;
        ...
        _medipen.UpdateAppearance((medipenSlot.Value, medipenComp), medipenSolutionComp.Value);
    ...
    private ContainerInfo? BuildInputContainerInfo(EntityUid? container)
    {
        ...
        if (!_solutionContainer.TryGetFitsInDispenser(container.Value, out _, out var solution))
            return null;
