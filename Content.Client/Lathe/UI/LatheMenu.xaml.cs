...
public sealed partial class LatheMenu : FancyWindow
{
    ...
    public LatheMenu()
    {
        ...
        Claim10Button.OnPressed += _ => ClaimMiningPointsAction?.Invoke(10);
        Claim50Button.OnPressed += _ => ClaimMiningPointsAction?.Invoke(50);
        Claim100Button.OnPressed += _ => ClaimMiningPointsAction?.Invoke(100);
        Claim500Button.OnPressed += _ => ClaimMiningPointsAction?.Invoke(500);
        ClaimAllButton.OnPressed += _ => ClaimAvailableMiningPoints();
        ClaimMiningPointsButton.OnPressed += _ => ClaimEnteredMiningPoints();
        MiningPointsAmount.IsValid = text => text.Length <= 10 && text.All(char.IsDigit);
        MiningPointsAmount.OnTextChanged += _ => UpdateMiningPointControls();
    }
    ...
    private void UpdateMiningPointControls()
    {
        ...
        MiningPointsAmount.Editable = siloConnected;
        Claim10Button.Disabled = !siloConnected || points.Points < 10;
        Claim50Button.Disabled = !siloConnected || points.Points < 50;
        Claim100Button.Disabled = !siloConnected || points.Points < 100;
        Claim500Button.Disabled = !siloConnected || points.Points < 500;
        ClaimAllButton.Disabled = !siloConnected || points.Points <= 0;
        ClaimMiningPointsButton.Disabled = !siloConnected || !uint.TryParse(MiningPointsAmount.Text, out var amount) ||
                                            amount == 0 || amount > points.Points;
    }
    ...
    private void ClaimAvailableMiningPoints()
    {
        if (_entityManager.TryGetComponent<MiningPointsComponent>(Entity, out var points) && points.Points > 0)
            ClaimMiningPointsAction?.Invoke((uint) points.Points);
    }

    private void ClaimEnteredMiningPoints()
    {
        if (uint.TryParse(MiningPointsAmount.Text, out var amount) && amount > 0)
