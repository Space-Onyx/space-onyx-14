...
public sealed partial class LatheMenu : DefaultWindow // <Onyx-LatheUi-edited>
{
    ...
    private Popup? _miningPointsPopup; // <Onyx-LatheMiningPoints>
    ...
    public event Action<uint>? ClaimMiningPointsAction;
    ...
    public LatheMenu()
    {
        ...
        ClaimMiningPointsButton.OnPressed += _ => OpenMiningPointsPopup(); // <Onyx-LatheMiningPoints-edited>
    }

    public void SetEntity(EntityUid uid)
    {
        Entity = uid;
        Title = _entityManager.GetComponent<MetaDataComponent>(Entity).EntityName; // <Onyx-LatheUi-edited>
    ...
    protected override void FrameUpdate(FrameEventArgs args)
    {
        ...
        UpdateMiningPointControls();
    }

    private void UpdateMiningPointControls()
    {
        ...
        MiningPointsBalance.Text = Loc.GetString("lathe-mining-points-balance", ("amount", points.Points));
        var siloConnected = _miningPoints.IsProcessorSiloConnected(Entity);
        MiningPointsNoConnectionWarning.Visible = !siloConnected;
        MiningPointsNoConnectionWarning.Text = GetSiloWarning();
        ClaimMiningPointsButton.Disabled = !siloConnected || points.Points <= 0; // <Onyx-LatheMiningPoints-edited>
    }

    private string? GetSiloWarning()
    {
        if (!_entityManager.TryGetComponent<OreSiloClientComponent>(Entity, out var client) ||
            client.Silo is not { } silo || !_entityManager.EntityExists(silo))
            return Loc.GetString("lathe-menu-mining-points-no-connection-warning");

        return _entityManager.GetComponent<TransformComponent>(Entity).MapID !=
               _entityManager.GetComponent<TransformComponent>(silo).MapID
            ? Loc.GetString("lathe-menu-mining-points-silo-not-on-same-grid")
            : null;
    }

    // <Onyx-LatheMiningPoints-edited>
    private void OpenMiningPointsPopup()
    {
        if (!_entityManager.TryGetComponent<MiningPointsComponent>(Entity, out var points) ||
            points.Points <= 0 || !_miningPoints.IsProcessorSiloConnected(Entity))
            return;

        _miningPointsPopup?.Close();

        var amountInput = new LineEdit
        {
            PlaceHolder = Loc.GetString("lathe-mining-points-amount"),
            MinWidth = 120,
        };
        amountInput.IsValid = text => text.Length <= 10 && text.All(char.IsDigit);

        var claimButton = new Button { Text = Loc.GetString("lathe-mining-points-claim") };
        var allButton = new Button { Text = Loc.GetString("lathe-mining-points-all") };
        var popup = new Popup
        {
            CloseOnClick = false,
            Children =
                ...
                new PanelContainer
                {
                    ...
                    Children =
                    {
                        new BoxContainer
                        {
                            ...
                            Children =
                            {
                                new Label { Text = Loc.GetString("lathe-mining-points-amount") },
                                new BoxContainer
                                {
                                    Orientation = BoxContainer.LayoutOrientation.Horizontal,
                                    SeparationOverride = 4,
                                    Children = { amountInput, claimButton, allButton },
                                },
                            },
                        },
                    },
        ...
        _miningPointsPopup = popup;
        popup.OnPopupHide += () => _miningPointsPopup = null;

        void UpdateClaimButton()
        {
            claimButton.Disabled = !uint.TryParse(amountInput.Text, out var amount) || amount == 0 ||
                                   !_entityManager.TryGetComponent<MiningPointsComponent>(Entity, out var current) ||
                                   amount > current.Points || !_miningPoints.IsProcessorSiloConnected(Entity);
        }

        amountInput.OnTextChanged += _ => UpdateClaimButton();
        UpdateClaimButton();

        claimButton.OnPressed += _ =>
        {
            if (!uint.TryParse(amountInput.Text, out var amount) || amount == 0 ||
                !_entityManager.TryGetComponent<MiningPointsComponent>(Entity, out var current) ||
                amount > current.Points || !_miningPoints.IsProcessorSiloConnected(Entity))
                return;

            ClaimMiningPointsAction?.Invoke(amount);
            popup.Close();
        };
        allButton.OnPressed += _ =>
        {
            if (!_entityManager.TryGetComponent<MiningPointsComponent>(Entity, out var current) ||
                current.Points <= 0 || !_miningPoints.IsProcessorSiloConnected(Entity))
                return;

            ClaimMiningPointsAction?.Invoke((uint) current.Points);
            popup.Close();
        };
