...
public sealed partial class SupermatterEntryContainer : BoxContainer
{
        ...
        if (isFocus)
        {
            if (focusData != null)
            {
                ...
                foreach (var gas in gases)
                {
                    var name = gas.Name;
                    var color = gas.Color;
                    var value = focusData.Value.AbsorbedMoles > 0
                        ? GetStoredGas(gas, focusData) / focusData.Value.AbsorbedMoles * 100
                        : 0;
    ...
    {
        if (focusData == null || !Enum.TryParse<Gas>(gas.ID, out var id))
            return 0;
