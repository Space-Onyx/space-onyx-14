...
namespace Content.Client._Onyx.Xenomorphs.RadialSelector;
...
public abstract partial class BasedRadialSelectorMenuBUI : BoundUserInterface
{
    ...
    protected void CreateMenu(List<RadialSelectorEntry> entries, RadialMenu menu, string parentCategory = "")
    {
        ...
        foreach (var entry in entries)
        {
            ...
            {
                var button = CreateButton(Loc.GetString(entry.Category.Name), _spriteSystem.Frame0(entry.Category.Icon));
    ...
    {
        if (ProtoManager.TryIndex(proto, out EntityPrototype? prototype))
