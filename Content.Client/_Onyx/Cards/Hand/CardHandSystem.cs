...
using Content.Shared._Onyx.Cards.Hand;
using Content.Shared._Onyx.Cards.Stack;
using Robust.Client.GameObjects;

namespace Content.Client._Onyx.Cards.Hand;
...
public sealed partial class CardHandSystem : EntitySystem
{
    ...
    private void UpdateSprite(EntityUid uid, CardHandComponent comp)
    {
        ...
        if (cardCount <= 0)
        {
            ...
            _sprite.LayerSetOffset((uid, sprite), 0, Vector2.Zero);
            _sprite.LayerSetScale((uid, sprite), 0, Vector2.One);
        }
        else if (cardCount == 1)
        {
            _cardSpriteSystem.TryHandleLayerConfiguration(
                ...
                {
                    _sprite.LayerSetRotation(sprt.AsNullable(), layerIndex, Angle.Zero);
                    _sprite.LayerSetOffset(sprt.AsNullable(), layerIndex, new Vector2(0, 0.10f));
                    _sprite.LayerSetScale(sprt.AsNullable(), layerIndex, new Vector2(comp.Scale));
                    return true;
                }
            );
        }
