...
public sealed partial class BankUiFragment : BoxContainer
{
    ...
    private readonly List<TransactionRecord> _filteredTransactions = [];
    ...
    private void UpdateHistoryList(IEnumerable<TransactionRecord> records)
    {
        ...
        foreach (var rec in records)
        {
            ...
            {
                Text = Loc.GetString("bank-program-ui-transaction-entry",
                    ("amount", rec.Amount),
                    ("color", rec.Amount >= 0 ? "green" : "red"),
                    ("description", rec.Description),
("timestamp", rec.Timestamp.ToLocalTime().ToString("g")),
                    ("comment", string.IsNullOrWhiteSpace(rec.Comment)
                        ? string.Empty
    ...
    public BankUiFragment()
    {
        ...
        HistorySearchLineEdit.OnTextChanged += _ =>
        {
            if (!_historyMode)
                return;
            var query = HistorySearchLineEdit.Text.Trim();
            ...
            {
                if (TransactionMatchesSearch(record, query))
                    _filteredTransactions.Add(record);
            }

            UpdateHistoryList(_filteredTransactions);
        };
    ...
    private void ApplyFlatButtonStyles()
    {
        var flat = FlatButtonStyle(Color.FromHex("#E5E5E5"), Color.FromHex("#D6D6D6"));

        Button[] buttons =
        [
            ExitHistoryButton, ChangePinShowButton, SavePinButton, ChangePinCancelButton,
            TransferShowButton, HistoryButton, TransferConfirmButton, TransferCancelButton,
            AccountLinkButton, LinkConfirmButton, LinkCancelButton
        ];

        foreach (var button in buttons)
        {
            button.StyleBoxOverride = flat;
        }
    }

    private static StyleBoxFlat FlatButtonStyle(Color background, Color border) => new()
    {
    ...
    {
        var source = lineEdit.Text;
        var text = source;
        var buffer = new char[Math.Min(source.Length, length)];
        var written = 0;
    ...
    {
        return record.Description.Contains(query, StringComparison.OrdinalIgnoreCase)
               || record.Amount.ToString().Contains(query, StringComparison.OrdinalIgnoreCase)
               || record.Comment?.Contains(query, StringComparison.OrdinalIgnoreCase) == true
               || (record.CounterpartyAccount?.Contains(query, StringComparison.OrdinalIgnoreCase) ?? false)
               || (record.CounterpartyName?.Contains(query, StringComparison.OrdinalIgnoreCase) ?? false);
    }
}
