...
namespace Content.Client._Onyx.ZLevels.Ladders;

[UsedImplicitly]
public sealed partial class CEZLevelLadderRadialMenuBUI : BoundUserInterface
{
    ...
    private List<RadialMenuOptionBase> ConvertEntries(List<RadialSelectorEntry> entries)
    {
        ...
        {
            if (entry.Prototype == null || entry.Icon == null)
                continue;
    ...
    {
        return _prototypeManager.TryIndex<EntityPrototype>(selected, out var proto)
            ? Loc.GetString(proto.Name)
            : Loc.GetString(selected);
    }
