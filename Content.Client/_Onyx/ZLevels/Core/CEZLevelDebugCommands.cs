...
public sealed partial class CEZDebugSelfCommand : LocalizedCommands
{
    ...
    public override void Execute(IConsoleShell shell, string argStr, string[] args)
    {
        ...
        if (_entityManager.TryGetComponent(uid, out CEZPhysicsComponent? zPhys))
            shell.WriteLine($"CEZPhysics: yes active={zLevels.IsBodyActive(uid)} processing={zLevels.IsBodyProcessing(uid)} local={Fmt(zPhys.LocalPosition)} vel={Fmt(zPhys.Velocity)} ground={Fmt(zPhys.CurrentGroundHeight)} sticky={zPhys.CurrentStickyGround} gravity={Fmt(zPhys.GravityMultiplier)} autoStep={zPhys.AutoStep}");
        else
            shell.WriteLine($"CEZPhysics: no active={zLevels.IsBodyActive(uid)}");

        if (xform.MapUid is { } mapUid && _entityManager.TryGetComponent(mapUid, out CEZLevelMapComponent? zMap))
        {
        ...
        if (xform.GridUid is not { } gridUid || !_entityManager.TryGetComponent(gridUid, out MapGridComponent? grid))
        {
        ...
        while (enumerator.MoveNext(out var anchoredNullable))
        {
            ...
            var line = $"- {anchoredMeta.EntityName} proto={anchoredMeta.EntityPrototype?.ID ?? "<none>"} uid={anchored}";
            if (_entityManager.TryGetComponent(anchored, out CEZLevelHighGroundComponent? highGround))
                line += $" highGround=true stick={highGround.Stick} corner={highGround.Corner} rot={xformSystem.GetWorldRotation(anchored)} curve=[{string.Join(", ", highGround.HeightCurve.Select(Fmt))}]";
...
public sealed partial class CEZDebugAllCommand : LocalizedCommands
{
    ...
    private static bool TryParseEnabled(string value, out bool enabled)
    {
        ...
        {
            case "1": case "on": case "true": case "enable": case "enabled":
                enabled = true;
                return true;
            case "0": case "off": case "false": case "disable": case "disabled":
                enabled = false;
                return true;
            default:
                enabled = false;
