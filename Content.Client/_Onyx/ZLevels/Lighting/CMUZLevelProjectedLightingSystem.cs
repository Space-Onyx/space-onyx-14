...
using Content.Client._Onyx.ZLevels.Lighting;
using Content.Shared._Onyx.ZLevels.Core;
...
public sealed partial class CMUZLevelProjectedLightingSystem : EntitySystem
{
    ...
    private readonly List<CEZLevelOpening> _tempOpenings = new();
    private readonly Dictionary<MapId, List<SourceLight>> _sourceLightBuckets = new();
    private readonly HashSet<MapId> _participatingSourceMaps = new();
    ...
    private readonly Dictionary<AcceptedCandidateBucketKey, List<int>> _acceptedCandidateBuckets = new();
    ...
    public override void FrameUpdate(float frameTime)
    {
        ...
        _acceptedCandidateCellSize = Math.Max(0.75f, maxRadius * 0.5f);
        ...
        var maxDepth = Math.Clamp(
            _config.GetCVar(CCVars.CEZLevelsVisibleBelow),
            0,
        ...
        BuildParticipatingSourceMaps(playerZLevelMap, playerMapComp.MapId, maxDepth);
    ...
    private void BuildSourceLightBuckets(Box2Rotated viewBounds, float minEnergy)
    {
        ...
        while (lightQuery.MoveNext(out var lightUid, out var light, out var lightXform))
        {
            ...
            if (_projectedQuery.HasComp(lightUid) ||
                ...
                !_participatingSourceMaps.Contains(lightXform.MapID) ||
                !light.Enabled ||
                light.Radius <= 0f ||
                light.Energy <= 0f ||
    ...
    private void BuildParticipatingSourceMaps(
        Entity<CEZLevelMapComponent?> playerMap,
        MapId playerMapId,
        int maxDepth)
    {
        _participatingSourceMaps.Clear();
        if (playerMapId != MapId.Nullspace)
            _participatingSourceMaps.Add(playerMapId);

        for (var depth = -maxDepth; depth <= 1; depth++)
        {
            if (depth == 0 ||
                !_zLevels.TryMapOffset(playerMap, depth, out var map) ||
                map is not { } resolved ||
                !_mapQuery.TryComp(resolved.Owner, out var mapComp) ||
                mapComp.MapId == MapId.Nullspace)
            {
                continue;
            }

            _participatingSourceMaps.Add(mapComp.MapId);
        }
    }
        ...
        float minEnergy)
    {
        ...
        foreach (var sourceLight in sourceLights)
        {
            ...
            {
                var openingCenter = opening.Center;
                var sourceToOpeningDistance = opening.Distance;
                // Reject imperceptible candidates before the much more expensive physics raycast.
    ...
    private bool TryAddStripCandidates(List<ProjectedLightCandidate> component)
    {
        ...
        for (var i = 0; i < sampleCount; i++)
        {
            ...
            AddAcceptedCandidate(candidate);
        }
    ...
    private void AddSeparatedCandidates(List<ProjectedLightCandidate> candidates, float energyScale)
    {
        ...
        foreach (var candidate in candidates)
        {
            ...
            AddAcceptedCandidate(scaledCandidate);
        }
    }

    private bool OverlapsAcceptedCandidate(ProjectedLightCandidate candidate)
    {
        var bucketKey = GetAcceptedCandidateBucketKey(candidate);
        for (var x = -1; x <= 1; x++)
        {
            for (var y = -1; y <= 1; y++)
            {
                var neighborKey = bucketKey with { X = bucketKey.X + x, Y = bucketKey.Y + y };
                if (!_acceptedCandidateBuckets.TryGetValue(neighborKey, out var indexes))
                    continue;

                foreach (var index in indexes)
                {
                    var accepted = _candidates[index];
    ...
    private void AddAcceptedCandidate(ProjectedLightCandidate candidate)
    {
        var index = _candidates.Count;
        _candidates.Add(candidate);
        var bucketKey = GetAcceptedCandidateBucketKey(candidate);
        if (!_acceptedCandidateBuckets.TryGetValue(bucketKey, out var bucket))
        {
            if (_acceptedCandidateBucketPool.Count == 0)
            {
                bucket = new List<int>();
            }
            else
            {
                bucket = _acceptedCandidateBucketPool[^1];
                _acceptedCandidateBucketPool.RemoveAt(_acceptedCandidateBucketPool.Count - 1);
            }

            _acceptedCandidateBuckets[bucketKey] = bucket;
        }

        bucket.Add(index);
    }

    private AcceptedCandidateBucketKey GetAcceptedCandidateBucketKey(ProjectedLightCandidate candidate)
    {
        return new AcceptedCandidateBucketKey(
            candidate.SourceLight,
            candidate.DepthOffset,
            (int) MathF.Floor(candidate.ProjectedCenter.X / _acceptedCandidateCellSize),
            (int) MathF.Floor(candidate.ProjectedCenter.Y / _acceptedCandidateCellSize));
    }

    private void ClearAcceptedCandidateBuckets()
    {
        foreach (var bucket in _acceptedCandidateBuckets.Values)
        {
            bucket.Clear();
            _acceptedCandidateBucketPool.Add(bucket);
        }

        _acceptedCandidateBuckets.Clear();
    }
    ...
    private EntityUid GetOrCreateProjectedLight(ProjectedLightCandidate candidate)
    {
        ...
        var key = new ProjectedLightKey(
            candidate.SourceLight,
            candidate.ReceivingMapId,
            candidate.OpeningGridUid,
            candidate.OpeningGridIndices);
    ...
    private readonly record struct AcceptedCandidateBucketKey(
        EntityUid SourceLight,
        int DepthOffset,
        int X,
        int Y);
    ...
    private sealed partial class ProjectedLightAlongAxisComparer : IComparer<ProjectedLightCandidate>
    {
        public Vector2 Axis;
