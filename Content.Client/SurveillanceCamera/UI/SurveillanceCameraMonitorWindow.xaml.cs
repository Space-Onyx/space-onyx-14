...
public sealed partial class SurveillanceCameraMonitorWindow : DefaultWindow
{
    ...
    public void SetZLevelViewEntity(EntityUid? entity)
    {
        CameraView.CEZLevelViewEntity = entity;
    }
    ...
    public void SetMap(EntityUid mapUid)
    {
        ...
        RefreshZLevelSelector(mapUid); // <Onyx-ZLevels>
    }

    // <Onyx-ZLevels>
    private void RefreshZLevelSelector(EntityUid rootGrid)
    {
        ZLevelSelector.RemoveAllChildren();
        if (!_entityManager.TryGetComponent<CEZLinkedGridComponent>(rootGrid, out var linked))
        {
            ZLevelSelector.Visible = false;
            return;
        }

        var levels = new SortedDictionary<int, EntityUid>(linked.PeerGrids) { [linked.Depth] = rootGrid };
        ZLevelSelector.Visible = levels.Count > 1;
        foreach (var (depth, grid) in levels)
        {
            var button = new Button { Text = depth.ToString(), Disabled = _mapUid == grid, Margin = new Thickness(2f) };
            button.OnPressed += _ =>
            {
                CameraMap.MapUid = _mapUid = grid;
                RefreshZLevelSelector(rootGrid);
            };
            ZLevelSelector.AddChild(button);
        }
    }
    // </Onyx-ZLevels>
